...
function __fc_cb_getplay_url(){
  //
  const _url = window.location.href;
  const _rand = Math.random();
  const _getplay_url = (_url.replace(/.*\/play\/(\d+?)-(\d+?)-(\d+?)\.html.*/, '/_getplay?aid=$1&playindex=$2&epindex=$3') + '&r=' + _rand);
  return _getplay_url;
}


function __fc_play_ep_scroll(){
  const _href_url = window.location.href;
  const _refresl = _href_url.match(/\/play\/(\d+?)-(\d+?)-(\d+?)\.html/);
  ...
  _sel_a.css('background', '#89B537');
  _sel_a.css('border', '1px solid #89B537');
...
const __fc_g_exXP = [''];
var __fc_g_isfullscn = false;
var __fc_g_new_playleft_id = null;
var __fc_margin_bak = '';


function __fc_playfull_set(_in_id, _in_title_on, _in_exXP){
  if (!navigator.userAgent.match(/(iPhone|iPod|Android|mobile|blackberry|webos|incognito|webmate|bada|nokia|lg|ucweb|skyfire)/i)) {
    ...
    $((".fullscn" + _in_exXP)).click(function() {
      if(!__fc_g_isfullscn){
        ...
        __fc_margin_bak = $('#'+_in_id + ' iframe').css('margin');
        $('#'+_in_id + ' iframe').css('margin', '0px');
        $('#'+_in_id).attr("id", _new_ID);

        //
        __fc_g_new_playleft_id = _new_ID;

        //
        __fc_g_isfullscn = true;
      }
      else {
        $((".fullscn" + '')).html(_in_title_on);
        $('#'+_in_id + ' iframe').css('margin', __fc_margin_bak);
        $(('#' + __fc_g_new_playleft_id)).attr("id", _in_id);

        //
        __fc_g_isfullscn = false;
      }
    });
  };
}

function __fc_exp_playfull_set(_in_id){
  for (var i = 0; i < __fc_g_exXP.length; i++){
    const p1 = (__fc_g_exXP[i] ? ('网页' + __fc_g_exXP[i] + 'P') : '网页全屏');
    const p2 = (__fc_g_exXP[i] ? ('-' + __fc_g_exXP[i] + 'p') : '');
    __fc_playfull_set(_in_id, p1, p2);
  }
}
