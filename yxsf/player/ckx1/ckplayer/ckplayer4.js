...
var __g_seek2 = 0;
var __g_played = false;
var __g_is_v2p = false;
var __g_is_mv2p = false;
var __g_is_1075_V = false;
var __g_is_rseek = false;
var __g_1075_lasttime_rseek = 0;
	...
	ckplayer.prototype = {
		...
		getVideo: function() {
			...
			if (this.html5Video) { //如果支持HTML5-VIDEO则默认使用HTML5-VIDEO播放器
				...
				if (isVideoH5 != null && thisPd != null) {
					...
					var html = '';
					var html_muted = '';
					if(this.vars.re_type == 'pre1'){
						html_muted = ' muted';
					}
					if (!this.isM3u8) {
						var vtype = this.vars.vtype;
						if(vtype){
							var v_vurl = this.vars.video[0][0];
							if(v_vurl.match(/^https*%/)){
								v_vurl = decodeURIComponent(v_vurl);
							}
							var v_source = '<source src="'+v_vurl+'" type="'+vtype+'">';
							html = '<video playsinline '+html_muted+' id="' + vid + '"' + ' width="100%" height="100%"' + autoplay + poster + loop + preload + controls + mobileautofull + track + '>' + v_source + '</video>';
						}else {
							html = '<video playsinline '+html_muted+' id="' + vid + '"' + src + ' width="100%" height="100%"' + autoplay + poster + loop + preload + controls + mobileautofull + track + '>' + source + '</video>';
						}
					} else {
						html = '<video playsinline '+html_muted+' id="' + vid + '" width="100%" height="100%"' + poster + loop + preload + controls + mobileautofull + track + '></video>';
					}
		...
		interFace: function() {
			...
			*/
			...
			var cLoadingFillRect = function() {
				...
				grad.addColorStop(0, '#000000'); //bBgColor
				var grad2 = cLoading.createLinearGradient(0, 0, 80, 60);
				grad2.addColorStop(1, '#000000'); //bOverColor
				var grad3 = cLoading.createLinearGradient(0, 0, 80, 60);
				grad3.addColorStop(1, '#000000'); //#FF9900
				var grad4 = cLoading.createLinearGradient(0, 0, 80, 60);
				grad4.addColorStop(1, '#000000'); //#CC3300
		...
		*/
		timeButtonMouseDown: function() {
			...
			var timeBoBgmousemove = function(event) {
				...
				var eventTime_2 = eventTime;
				if(thisTemp.vlt_len){
				 	eventTime_2 = thisTemp.vlt_len * (eventTime / thisTemp.V.duration);
				}
				var buttonPromptObj = {
					title: thisTemp.formatTime(eventTime_2),
					x: eventX,
					y: timeBoBgXY['y']
				};
				thisTemp.promptShow(false, buttonPromptObj);
		...
		controlBarHide: function(hide) {
			...
			var controlBarShow = function(show) {
				if (show && !cShow && thisTemp.controlBarIsShow) {
					...
					//
					thisTemp.timeProgress(thisTemp.V.currentTime, thisTemp.V.duration);
		...
		*/
		timeProgress: function(time, duration) {
			if(this.vlt_len){
				time = (time - this.vlt_l);
				if(time < 0){time = 0;}
				duration = this.vlt_len;
			}
			if (!this.showFace) {
				return;
			}
		...
		*/
		timeTextHandler: function() { //显示时间/总时间
			...
			if(this.vlt_len){
				duration = this.vlt_len;
				time = this.V.currentTime - this.vlt_l;
				if(time < 0){time = 0;}
			}
			//
			if (isNaN(duration) || parseInt(duration) < 0.2) {
		...
		*/
		videoSeek: function(time) {
			...
			if (time >= 0) {
				...
				else if(this.vlt_len){
					const posRev = (time / this.V.duration);
					newPosTime = (this.vlt_l + (this.vlt_len*posRev));
				}
				this.V.currentTime = newPosTime;
				this.sendJS('seekTime', newPosTime);
			}
		},
		/*
			接口函数
...
var _vurl = __get_url_param(_curr_url, 'url');
var _ud = __get_url_param(_curr_url, 'ud');
var _vtype = __get_url_param(_curr_url, 'vtype');
var _v2p = __get_url_param(_curr_url, 'v2p');
var _mv2p = __get_url_param(_curr_url, 'mv2p');
function GetVUrlx(in_vurl){
	var _vurl = in_vurl;
	if(_vurl){
		if(_ud != '1'){
			_vurl = decodeURIComponent(_vurl);
		}
	}
	return _vurl;
}
if(_vtype){
	_vtype = decodeURIComponent(_vtype);
}
if(_vurl){
	_vurl = GetVUrlx(_vurl);
}
if('1' == _v2p){
	__g_is_v2p = true;
}
...
const hf_vurl_pre = (_vurl ? _vurl : "");
const hf_vurl = (__is_tieba_video(hf_vurl_pre) ? __set_tieba_video_rpt(hf_vurl_pre) : hf_vurl_pre);
const __g_vurl0 = hf_vurl;
const vlt_l = Number(__get_url_param(_curr_url, 'vlt_l'));
const vlt_r = Number(__get_url_param(_curr_url, 'vlt_r'));
__g_seek2 = __key_dec_vlt(vlt_l);
//
var ck_volume = getCookie2('ck_volume');
ck_volume = (ck_volume ? parseFloat(ck_volume) : 0.8);
//
__g_is_1075_V = isVideoPre(hf_vurl);
__g_is_rseek = IsRSeek(hf_vurl);
...
var videoObject = {
	...
	poster: 0,
	volume: ck_volume,
	seek: __key_dec_vlt(vlt_l),
	flashplayer: (Number(__get_url_param(_curr_url, 'flash')) ? true : false),
	loaded: 'loadedHandler', //当播放器加载后执行的函数	
	video: [
		[hf_vurl, 'video/mp4', 'default', 0],
	]
};
var videoObject_pre1 = {
	container: '#video-pre1', //容器的ID或className
	variable: 'player',//播放函数名称
	autoplay:false,
	poster: 0,
	volume: 0,
	...
	seek: __key_dec_vlt(vlt_l),
	flashplayer: (Number(__get_url_param(_curr_url, 'flash')) ? true : false),
	loaded: 'loadedHandler_pre1', //当播放器加载后执行的函数	
	video: [
		[hf_vurl, 'video/mp4', 'default', 0],
	]
};
var player = new ckplayer(videoObject);
var player_pre1 = undefined;
if(__g_is_1075_V){
	player_pre1 = new ckplayer(videoObject_pre1);
}

__init_vlt(player, vlt_l, vlt_r);

//
function __init_vlt(player, l, r){
	if(true){
		f = (function (name){
...
function loadedHandler_pre1() {

}

function volumeChangeHandler(vol) {
	setCookie2('ck_volume', parseFloat(vol.toString()));
}

function isVideoPre(in_vurl){
	...
	//
	if(in_vurl.indexOf('//photovideo.photo.qq.com/1075_') < 0){
		if(in_vurl.indexOf('//photovideo.photo.qq.com/1074_') < 0){
			return false;
		}
	}
...
//
setTimeout(function(){
	if(!__g_played){ // 6秒未播放重试
		if(!__is_tieba_video(__g_vurl0)){
			errorHandler2(0);
		}
	}
}, 6000);

function Is1075TimeCtrl() {
	if(player.V){
		var v_dur = player.V.duration;
		if(v_dur && v_dur > 120){
			return true;
		}
	}
...
function IsRSeek(in_vurl){
	//
	if(in_vurl.match(/^https*%/)){
		in_vurl = decodeURIComponent(in_vurl);
	}

	//
	if(in_vurl.indexOf('.f20.mp4') > 0){
		if(in_vurl.indexOf('.qq.com/1075_') > 0 || in_vurl.indexOf('.qq.com/1074_') > 0){
			return true;
		}
	}
	//
...
//
var __g_setptime_last = 0;
var __g_setptime_cnt = 0;
function SetPTime(in_time){
	if(typeof CK_SPT == 'undefined'){
		return false;
	}
	//
	var c_time = parseInt((new Date()).getTime() / 1000);
	if(!__g_setptime_last){
		__g_setptime_last = c_time;
	}
	if(c_time - __g_setptime_last > 5){
		__g_setptime_last = c_time;
		__g_setptime_cnt += 1;
		//
		if(__g_setptime_cnt > 5){
			var ptcnt = getCookie2('ptime_cnt');
			if(!ptcnt){
				ptcnt = 0;
			}
			ptcnt = Number(ptcnt);
			ptcnt += 5;
			setCookie2('ptime_cnt', ptcnt);
		}
	}
	//
	return true;
...
//
var __g_pre1_time_cnt = 0;
function timeHandler(time) {
	if(!__g_played){
		if(__g_is_1075_V && Is1075TimeCtrl()){
			player_pre1.videoPlay();
		}
		__g_played = true;
	}
	...
	//
	if(__g_is_1075_V && Is1075TimeCtrl()){
		if(__g_pre1_time_cnt++ % 3 == 0){
			var pre1_time = player_pre1.V.currentTime;
			if(pre1_time > 1){
				if(pre1_time - time > 60){
					player_pre1.videoSeek(time+3);
				}
				else if(pre1_time - time > 40){
					player_pre1.changePlaybackRate(1);
				}
				else if(pre1_time - time < -10){
					player_pre1.videoSeek(time+3);
				}
				else {
					player_pre1.changePlaybackRate(5);
				}
			}
		}
	}
	...
	if(__g_is_rseek){
		var c_time = parseInt((new Date()).getTime() / 1000);
		if(!__g_1075_lasttime_rseek){
			__g_1075_lasttime_rseek = c_time;
		}
		if(c_time - __g_1075_lasttime_rseek > (60*5)){
			__g_1075_lasttime_rseek = c_time;
			player.V.currentTime = (player.V.currentTime - 0.05);
			console.log('=== rseek');
		}
	}
	...
	if(time < (player.vlt_l - 1)){
		player.V.currentTime = player.vlt_l;
	}
	else if(time > player.vlt_r){
		player.videoPause();
		player['ended'] = true;
	}
}
...
function __rget_url_rev_vurl(purl, vurl){
	purl = __yx_purl_rev(purl);
	var r2url = (purl + vurl);
	//
	var vurl = __get_url_param(r2url, 'url');
	vurl = GetVUrlx(vurl);
	return vurl;
}

function GetXUU(in_url) {
  var xuu = (in_url.indexOf('?') >= 0 ? '&' : '?');
  return xuu;
}
...
function __rget_chk_eql_vurl(vurl0, vurl){
	vurl0 = SetUrlQuery(vurl0, 't', '');
	vurl = SetUrlQuery(vurl, 't', '');
	if(vurl0 == vurl){
		return true;
	}
	//
...
function __ck_getset_play(cb_cnt){
	const _curr_url = window.location.href;
	const _rget_url = __get_url_param(_curr_url, 'getplay_url');
	if(!_rget_url){
		return;
	}
	const _getplay_url = decodeURIComponent(_rget_url) + '&r2=1';
	//
	if(dettchk()){
		$('#id-no-referrer').attr('content', 'always');
		$.get(_getplay_url, function(_in_data, _in_status){
			if('err:timeout' == _in_data){
				if(cb_cnt > 0){
				  __getplay_pck();
				  __getplay_pck2();
				  return __ck_getset_play(cb_cnt-1);
				}else {
				  return;
				}
			}
			...
			//
			_in_data = __getplay_rev_data(_in_data);

			//
			$('#id-no-referrer').attr('content', 'no-referrer');
			...
			const _hf_vurl = __rget_url_rev_vurl(_purl, _vurl);
			if(_hf_vurl && !__rget_chk_eql_vurl(__g_vurl0, _hf_vurl)){
				//
				__g_is_1075_V = isVideoPre(_hf_vurl);
				__g_is_rseek = IsRSeek(_hf_vurl);

				//
				__g_seek2 = __key_dec_vlt(vlt_lr[0]);
				videoObject['seek'] = __key_dec_vlt(vlt_lr[0]);
				videoObject['autoplay'] = false;
				videoObject['video'] = [[_hf_vurl, 'video/mp4', 'default', 0]];
				player = new ckplayer(videoObject);
				__init_vlt(player, vlt_lr[0], vlt_lr[1]);

				//
				if(__g_is_1075_V){
					videoObject_pre1['seek'] = __key_dec_vlt(vlt_lr[0]);
					videoObject_pre1['autoplay'] = true;
					videoObject_pre1['video'] = _hf_vurl;
					if(undefined == player_pre1){
						player_pre1 = new ckplayer(videoObject_pre1);
					}
					else {
						player_pre1.newVideo(videoObject_pre1);
					}			
				}
			}
		});
...
//
var _err_getplay_cnt = 0;
function errorHandler2(in_timeout) {
	//
	if(__tieba_err_replay()){
		return;
	}

	//
	if(_err_getplay_cnt < 1){
		++_err_getplay_cnt;
		//
		setTimeout(function(){
...
var TIEBA_ERR_CNT = 0;
function __is_tieba_video(vurl){
	if(vurl.match(/\/\/[^\/]+?\.baidu\.com\//)){
		return true;
	}
	return false;
}
function __set_tieba_video_rpt(vurl){
	return SetUrlQuery(vurl, 'rpt', new Date().getTime());
}
function __tieba_err_replay(){
	if(!__is_tieba_video(__g_vurl0)){
		return false;
	}

	//
	if(TIEBA_ERR_CNT++ > 1000){
		return true;
	}
	...
	//
	var new_vurl = __set_tieba_video_rpt(__g_vurl0);
	videoObject['video'] = new_vurl;
...
if(__is_tieba_video(__g_vurl0)){
	//
	$("#divtip-tieba").css("display", "block");
	setTimeout(function(){
		$("#divtip-tieba").fadeOut(2000);
	}, 10000);
}

function errorHandler(){
	//
	return errorHandler2(5000);
}
